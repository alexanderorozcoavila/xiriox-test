...
##############################################
x = 640 # ancho de las imágenes de las cámaras para visualizar
y = 360 # alto de las imágenes de las cámaras para visualizar
##############################################
...
for i in range(0,8): # se inicializan ambos array
        ...
        if respuesta == 0:
            ...
            while True: # ciclo para ir recibiendo los frames
                ...
                if ret: # si es recibido de forma correcta se redimensiona el frame
                    ...
                    #print(w, h)
                    cv2.putText(ret_frame, "CAM "+str(var2+1)+" 10.1.1."+var+"  - FPS: "+str(fps)+'    '+bitrate_cam+'    '+str(w)+'x'+str(h), (7, 42), cv2.FONT_HERSHEY_DUPLEX, 1.1, (0,0,0), 2,lineType=cv2.LINE_AA)
                    cv2.putText(ret_frame, "CAM "+str(var2+1)+" 10.1.1."+var+"  - FPS: "+str(fps)+'    '+bitrate_cam+'    '+str(w)+'x'+str(h), (5, 40), cv2.FONT_HERSHEY_DUPLEX, 1.1, (255,255,255), 2,lineType=cv2.LINE_AA)
...
def principal():
    ...
    while True:
        ...
        img_vacia = Image.new("RGB", (x,y), (25, 0, 51))
        img = Image.new("RGB", (x,y), (25, 0, 51))
        ImageDraw.Draw(img).text((10,20), fecha+" "+hora, fill='white', font=ttf)
        ImageDraw.Draw(img).text((10,45), id_dri, fill='white', font=ttf)
        ImageDraw.Draw(img).text((10,70), id_embarcacion, fill='white', font=ttf)
        ImageDraw.Draw(img).text((10,95), "LAT: "+gps[3], fill='white', font=ttf)
        ImageDraw.Draw(img).text((10,120), "LON: "+gps[4], fill='white', font=ttf)
        ImageDraw.Draw(img).text((10,145), "RUMBO: "+gps[6], fill='white', font=ttf)
        ImageDraw.Draw(img).text((10,170), "VEL (NUDOS): "+gps[5], fill='white', font=ttf)
        # recuadro (0,4) que muestra la información de los discos duros
        img2 = Image.new("RGB", (x,y), (25, 0, 51))
        ...
        try:
            ...
            print(e)
        try:
            disco_duro3 = psutil.disk_usage('/media/xirioxinf/Ext3')#cambiar directorio para detección de disco
            espacioDisponible3 = disco_duro3[1] #Mostrar espacio libre en GB
        except Exception as e:
            disco_duro3 = [0]
            espacioDisponible3 = 0
            print(e)
        try:
            espacioTotal = disco_duro[0]+disco_duro2[0]+disco_duro3[0]
            espacioOcupado = espacioDisponible+espacioDisponible2+espacioDisponible3
        except Exception as e:
            raise e
        ...
        arrayAlarmas.close()
        try:
            ImageDraw.Draw(img2).text((10,45), str(round((espacioOcupado*100)/espacioTotal,2)) +" % HDD Ocupado", fill='white', font=ttf)
        except Exception as e:
        ...
        if dataa == "":
            ...
            img.paste(img_boton, (10,300), img_boton) # pega la imágen sobre el background creado anteriormente (img2)
            ImageDraw.Draw(img).text((60,310), "OK", fill='white', font=ttf)
        else:
            img_boton = Image.open("/home/xirioxinf/Documentos/descarte_xiriox/interfaz/img/boton_azul.png")
            img.paste(img_boton, (10,300), img_boton) # pega la imágen sobre el background creado anteriormente (img2)
            ImageDraw.Draw(img).text((60,310), "ALARMA", fill='white', font=ttf)
        ...
        horizontal1 = np.concatenate((arr_frames[0], arr_frames[1], arr_frames[2]), axis=1)
        horizontal2 = np.concatenate((arr_frames[3], arr_frames[4], arr_frames[5]), axis=1)

        horizontal3 = np.concatenate((arr_frames[6], img, img2), axis=1)
        ...
        final = np.concatenate((horizontal1,horizontal2), axis=0) # se juntan todos los frames en una sola ventana
        total = np.concatenate((final,horizontal3), axis=0) # se juntan todos los frames en una sola ventana
