...
# print('ultimaCarpeta: ')
# print(ultimaCarpeta)
# print(dir_encrypt+'/'+fecha)

# if ultimaCarpeta != dir_encrypt+'/'+fecha and dir_encrypt != '/home/xirioxinf/ERROR':
#     print('Distintos')    
#     listaCarpetas.write('\n'+dir_encrypt+'/'+fecha)
# else:
...
def ultimo_video(num, fecha):
    ...
    files = os.listdir(dir_encrypt+'/'+fecha+'/CAM'+num+'/') # Obtiene todos los archivos del directorio de la cámara
...
def grabando(num_ip, num_cam):
    ...
    path = dir_encrypt+'/'+fecha+'/CAM'+num_cam+'/' # directorio de la cámara
    ...
    proc = subprocess.Popen(['python3 /home/xirioxinf/Documentos/descarte_xiriox/scripts/srtEncrypt.py '+str(num_cam)], shell=True)
    # ejecuta la consola con el comando ffmpeg
    os.system('ffmpeg -t '+tiempo_grabacion_videos+' -rtsp_transport tcp -i rtsp://admin:xiriox3000@10.1.1.'+num_ip+':554 -acodec copy -vcodec copy '+FILE_OUTPUT+' > /home/xirioxinf/Documentos/descarte_xiriox/grabar/output/output'+num_cam+'.csv 2>&1')
    ...
    hasher_md5(dir_encrypt+'/'+fecha+'/CAM'+num_cam+'/'+ult_video, dir_encrypt+'/'+fecha+'/CAM'+num_cam+'/'+nombre_ult_video+".md5")
    ###################################################################################
